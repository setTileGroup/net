...
var plugins={
	...
	healthShow:{Enable:false,bar:null,text:null,text2:null,window:null,name:"",max:0,id:0,time:0,showBackgruond:false,showName:true,showBar:true,showPoint:false,autoDismissTime:10}
}
...
{/*
	...
	{
	function tree_destoryPlanes(i,j,k,d){;
	}
		var id=Level.getTile(x,y,z),data=Level.getData(x,y,z);
		if((id==17 || id==162) && plugins.tree.mode )
		{
			var l=0;
			var same=false; 
			...
			while(1)
			{
				Level.setTile(lastpos[0],lastpos[1],lastpos[2]);
				if(i<=1)
				{
					if(j<=1)
					{
						if(k<=1)
						{
							if( plugins.tree.autoRe&& 17==Level.getTile(lastpos[0]+i,lastpos[1]+j,lastpos[2]+k)==17||Level.getTile(lastpos[0]+i,lastpos[1]+j,lastpos[2]+k)==162)   //是木头方块则加入数组
							{
								same=false;
								for(l in blocks)
								{
									if( plugins.tree.autoRe && blocks[l][0]==lastpos[0]+i&&woodblocks[l][1]==lastpos[1]+j&&blocks[v][2]==lastpos[2]+k)
										same=true;
								}
								if(!same)
								{
...
function newLevel()//进入世界
{
	...
	if(first==true)
	{
		...
		{
			showLogo(logo1,"欢迎使用超级辅助！\n作者:风鸟 贴吧ID:ca765569811\nQQ:765569811 请尊重作者版权！");
			print(Bugmsg);
			print(Bugmsg);
		}
		loaded=true;
